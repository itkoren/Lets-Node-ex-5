...
var server = http.createServer(function(req, res) {
    ...
    function complete(err, items, hasUTube) {
        completed++;
                ...
                sentiment(item.text, function(err, score) {
                    ...
                    setImmediate(function() {
                        ...
                        if (i < (results.length - 1)) {
                            ...
                            if (!hasUTube) {
                                // Querying the google API for the supplied term
                                request("https://gdata.youtube.com/feeds/api/videos?max-results=5&alt=json&orderby=published&v=2&q=" + encodeURIComponent(qs.term), function(error, response, body) {
                                    // Storage objects
                                    var results;
                                    var items = [];
                                    var i = 0;

                                    // Etheration function for parsing the score of each tweet returned from the Utube API
                                    // Using the sentiment module API
                                    function reparse(result, results, items, callback) {
                                        var item = {
                                            src: "UTube",
                                            text: result.title.$t,
                                            score: 0
                                        ...
                                        sentiment(item.text, function (err, score) {
                                            ...
                                            setImmediate(function () {
                                                // Check if etheration should continue
                                                if (i < (results.length - 1)) {
                                                    reparse(results[++i], results, items, callback);
                                                }
                                                else {
                                                    callback(null, items);
                                                }
                                    ...
                                    if (body) {
                                        ...
                                        // Start parsing
                                        if (0 < results.length) {
                                            reparse(results[i], results, items, complete);
                                        }
                                    }
                                });
                            }
                            callback(null, items, hasUTube);
                        }
                    });
                });
            }
